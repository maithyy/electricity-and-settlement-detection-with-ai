...
class Encoder(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, depth: int, kernel_size: int, pool_size: int):
        ...
        convolutions.append(nn.Conv2d(in_channels, out_channels, kernel_size=(kernel_size, kernel_size), padding=kernel_size//2))
        # append a ReLU layer
        ...
        for _ in range(depth-1):
            ...
            convolutions.append(nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=(kernel_size, kernel_size), padding=kernel_size//2))
            # create a relu
            convolutions.append(nn.ReLU())
    ...
    def forward(self, img):
        ...
        """
        # self.convs
        img = self.convs.forward(img)
        
        # self.pool
        img = self.pool.forward(img)
...
class SegmentationCNN(nn.Module):
    def __init__(self, in_channels: int, out_channels: int,  depth: int = 2, embedding_size: int = 64,
        ...
        self.decoder = nn.Conv2d(in_channels=embedding_size,
