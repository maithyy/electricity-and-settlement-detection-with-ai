...
def restitch_and_save_to_tiff(
    ...
    subtile, prediction = restitch_eval(
        processed_dir=options.processed_dir / train_or_val,
    ...
    if is_random_forest:
        # random forest predictions already given and 0s and 1s
        y_pred = prediction[0][0].astype(int)
    else:
        # segcnn, fcnresnet, and unet predictions given as class probabilities
    ...
    if results_dir is None:
        ...
        tiff.imwrite(f"{results_dir}/{parent_tile_id}.tif", y_pred)
        print(f'saved {parent_tile_id} to tiff')
        plt.close()

def restitch_and_plot_randforest(
    options,
    datamodule,
    model,
    parent_tile_id: str,
    ...
    # Uncomment to print accuracy values:
    # ground_truth = subtile.ground_truth.values[0][0] - 1
    # predictions = prediction[0][0]
    # accuracy = np.sum(ground_truth == predictions) / predictions.size
    # print("accuracy", accuracy)

    y_pred = prediction[0][0].astype(int)
    cmap = matplotlib.colors.LinearSegmentedColormap.from_list(
        "Settlements", np.array(["#808080", "#5C4033", "#ffff00", "#b266ff"]), N=4
    )
