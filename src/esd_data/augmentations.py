...
class Blur(object):
    ...
    def __call__(self, sample: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        blurred_img = apply_per_band(img, lambda x: cv2.blur(x, (kernel, kernel)))

        # return the {X : img, y : mask}
        return {'X': blurred_img, 'y': mask}
...
class AddNoise(object):
    ...
    def __call__(self, sample):
        ...
        rand_std = random.uniform(0, self.std_lim)
        ...
        noise = np.random.normal(self.mean, rand_std, img.shape)
