...
class ESDDataModule(pl.LightningDataModule):
    ...
    def load_and_preprocess(
        ...
        if IS_BINARY_SEGMENTATION:
            return preprocessed_data_array_list, load_satellite(tile_dir, SatelliteType.GT), load_satellite(tile_dir, SatelliteType.GT_COMBINED)
        else:
    ...
    def prepare_data(self) -> None:
        ...
        if not self.processed_dir.is_dir() or len(list(self.processed_dir.iterdir())) == 0:

            # get the a list of the tile directories from the raw directory (use .glob() for simplicity)
            tile_dirs = [tile_dir for tile_dir in self.raw_dir.glob("Tile*")]
            ...
            # tile_dirs_train, tile_dirs_val = train_test_split(tile_directories, test_size=1-self.train_size, random_state=self.seed)
            ...
            for tile_dir in tqdm(list(tile_dirs_train), desc="Processing train tiles"):
                ...
                if IS_BINARY_SEGMENTATION:
                    data_array_list, gt, gt_combined = self.load_and_preprocess(tile_dir)
                else:
                    data_array_list, gt = self.load_and_preprocess(tile_dir)

                # create a subtile, passing the data array list, gt data array, and the slice size
                if IS_BINARY_SEGMENTATION:
                    subtile = Subtile(data_array_list, gt_combined, self.slice_size)
                else:
                    subtile = Subtile(data_array_list, gt, self.slice_size)
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        return torch.utils.data.DataLoader(
            ...
            pin_memory=True, prefetch_factor=2 # added these after running out of memory from large batch size
        )
    ...
    def test_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        """
        # create the torch.utils.data.Dataloader for the test_dataset, passing the batch size
        # and collate_fn
        return torch.utils.data.DataLoader(
