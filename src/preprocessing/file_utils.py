...
def process_viirs_filename(file_path: Path) -> Tuple[str, str]:
    ...
    date = file_name[len("DNB_VNP46A1_A"):-4]
...
def process_s1_filename(file_path: Path) -> Tuple[str, str]:
    ...
    """
    file_name = file_path.name
    date = file_name[len("S1A_IW_GRDH_"):-7]
...
def process_landsat_filename(file_path: Path) -> Tuple[str, str]:
    ...
    """
    file_name = file_path.name
    date = file_name[len("LC08_L1TP_"):len("LC08_L1TP_yyyy-mm-dd")]
    band = file_name[len("LC08_L1TP_yyyy-mm-dd_B"):-4]
    return (date, band)
...
def process_ground_truth_filename(file_path: Path) -> Tuple[str, str]:
    ...
    Tuple[str, str]
        A tuple containing the date and band.
    """
    # return (datetime.min.date().strftime("%Y-%m-%d"), "0")
    return ("0001-01-01", "0")

def process_ground_truth_combined_filename(file_path: Path) -> Tuple[str, str]:
    """
    This function takes in the file_path of the ground truth file and returns
    ("0", "0"), as there is only one ground truth file. The date is set to the
    datetime.min.date() and converted to a string.
...
def get_filename_pattern(satellite_type: SatelliteType) -> str:
    ...
    patterns = {
        ...
        "landsat": "LC08_L1TP_",
        "gt": "groundTruth.tif",
        "gt_combined": "groundTruth_combined.tif"
    }

    if satellite_type.value in patterns:
        return patterns[satellite_type.value]
    
    print('get_filename_pattern: Satellite type', satellite_type.value, 'not a valid type')
    return None
...
def get_satellite_files(tile_dir: Path, satellite_type: SatelliteType) -> List[Path]:
    ...
    for path in tile_dir.iterdir():
        name = path.name
        if not path.is_dir() and pattern in name and name[-4:] == ".tif":
            files.append(path)
...
def get_grouping_function(satellite_type: SatelliteType) -> Callable:
    ...
    patterns = {
        ...
        "gt_combined": process_ground_truth_combined_filename
    }
    if satellite_type.value in patterns:
        return patterns[satellite_type.value]
...
def get_parent_tile_id(tile_dir: Path) -> str:
    ...
    """
    return tile_dir.name # or tile_dir.parent.name
...
def read_satellite_file(satellite_file: Path) -> np.ndarray:
    ...
    """
    return tifffile.imread(satellite_file, dtype=np.float32)
...
def load_satellite(tile_dir: Path, satellite_type: SatelliteType) -> xr.DataArray:
    ...
    date_dimension = [] # empty list to store the data along the date dimension
   
    for date in dates: # iterate over the dates

        band_dimension = [] # empty list to store the data along the band dimension
        
        for band in bands: # iterate over the bands

            for file in file_names: # iterate over the files in file_names
            
                # get the date of the file and the band of the file using the grouping function
                d,b = grouping_function(file)
    ...
    xarray = xr.DataArray(
        stacked_array, dims=("date", "band", "height", "width"), 
        coords={"date": dates, "band": bands, "height": range(stacked_array.shape[-2]), "width": range(stacked_array.shape[-1])})

    # add the satellite_type, tile_dir, and parent_tile_id as attributes to the xarray
    xarray.attrs['satellite_type'] = satellite_type.value
    xarray.attrs['tile_dir'] = str(tile_dir)
    xarray.attrs['parent_tile_id'] = parent_tile_id
...
def load_satellite_list(
    ...
    for satellite_type in satellite_type_list:
        satellites.append(load_satellite(tile_dir, satellite_type))
    return satellites
