...
class Subtile:
    ...
    def _save_label(
        ...
        if IS_BINARY_SEGMENTATION:
            subtiled_ground_truth.to_netcdf(
                ...
                / f"{x}_{y}"
                / f"{SatelliteType.GT_COMBINED.value}.nc"
            )
        else:
            subtiled_ground_truth.to_netcdf(
                subtile_directory
        ...
        if has_gt:
            if IS_BINARY_SEGMENTATION:
                gt_data_array = xr.load_dataarray(
                    directory_to_load / f"{SatelliteType.GT_COMBINED.value}.nc"
                )
            else:
                gt_data_array = xr.load_dataarray(
    ...
    def restitch(
        ...
        if IS_BINARY_SEGMENTATION:
            satellite_type_list_with_gt = satellite_type_list + [SatelliteType.GT_COMBINED]
        else:
